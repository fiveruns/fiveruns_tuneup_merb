...
function Boxy(element, options) {
    ...
    this.boxy.css('display', 'none').appendTo(page.body);
    this.toTop();
...
$.extend(Boxy, {
    ...
    ask: function(question, answers, callback, options) {
        ...
        page.find('input[type=button]', buttons).click(function() {
            var clicked = this;
...
Boxy.prototype = {
    ...
    getInner: function() {
        return page.find('.boxy-inner', this.boxy);
    },
    
    // Returns a $ object wrapping the boxy content region.
    // This is the user-editable content area (i.e. excludes titlebar)
    getContent: function() {
        return page.find('.boxy-content', this.boxy);
    },
    ...
    centerX: function() {
        if (this.options.fixed) {
          ...
          var s = $.browser.msie ? page.documentElement.scrollLeft : _window.pageXOffset;
        }
        var v = [s, $(_window).width()];
    ...
    centerY: function() {
        if (this.options.fixed) {
          ...
          var s = $.browser.msie ? page.documentElement.scrollTop : _window.pageYOffset;
        }
        var v = [s, $(_window).height()];
    ...
    hide: function(after) {
        ...
        var self = this;
        if (this.options.modal) {
            page.find('.boxy-modal-blackout').animate({opacity: 0}, function() {
                $(this).remove();
            });
        }
        ...
        if (this.options.title) {
            ...
            if (this.options.draggable) {
                ...
                if (!Boxy.dragConfigured) {
                    page.mousemove(Boxy._handleDrag);
    ...
    _setupBehaviours: function() {
        var self = this;
        page.find('.close', this.boxy).click(function() {
            self.hide();
            return false;
